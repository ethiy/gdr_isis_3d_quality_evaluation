...
\begin{document}
	\begin{tikzpicture}
		\node[visible on=<1-2>] (dsm) at (0,0) {\includegraphics[width=3cm]{images/features/dsm}};
		...
		\path (minus.east) node[anchor=west, visible on=<2>] (model) {\includegraphics[width=3cm]{images/features/height_map}};
        \path (model.south) node[anchor=north, text width=3cm, align=center, visible on=<2>] (model_legend) {\scriptsize Model height};

        \path (minus) node[visible on=<3>] (residual_colorbar) {\includegraphics[width=3cm]{images/features/residual_colbar}};
        ...
        \path (dsm.west) node[anchor=west, visible on=<4->] (residual) {\includegraphics[width=1.5cm]{images/features/residual}};
